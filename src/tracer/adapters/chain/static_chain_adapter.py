...
class StaticChainAdapter(ChainDataPort):
    def __init__(self, 
        ...
        self._ts_to_block = ts_to_block or 0
    ...
    def get_token_meta(self, token_address):
        return self._meta.get(token_address.lower(),TokenMeta(token_address, None, None, None, None))
    ...
    def iter_erc20_transfers(self, address, start_block, end_block, sort = "asc", token_address = None):
        ...
        items = [
           ...
           and t.from_address.lower() == ad or t.to_address==ad
           and  (token is  None or t.token_address == token)
        ]
        if sort=="desc":
