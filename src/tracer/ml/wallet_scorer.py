...
def _label_from_prob(prob: float) -> str:
    if prob >= float(settings.ML_WALLET_HIGH_THRESHOLD):
        return "HIGH_RISK"
    if prob >= float(settings.ML_WALLET_MEDIUM_THRESHOLD):
        return "MEDIUM_RISK"
    if prob > 0:
        return "LOW_RISK"
    return "UNKNOWN"
...
class WalletMLScorer:
    def __init__(self, model_path: str = settings.ML_WALLET_MODEL_PATH) -> None:
        self._model_path = model_path
        self._model = load_model(model_path)

    def score(self, address: str, graph: Graph) -> Optional[Dict[str, object]]:
